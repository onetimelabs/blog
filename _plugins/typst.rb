...
require 'json'
  ...
  def self.compile_typst_svg(math_code, color, prefix = 'typst', extra_preamble = '', display_math: false, size: nil)
    result = nil
    Tempfile.create([prefix, '.typ']) do |file|
      file.write("#set page(width: auto, height: auto, fill: none, margin: 2pt)\n")
      size_rule = size ? "size: #{size}, " : ""
      file.write("#set text(#{size_rule}fill: rgb(\"#{color}\"))\n")
      ...
      file.write(display_math ? "$ #{math_code} $" : "$#{math_code}$")
      file.rewind
  ...
  class TypstBlock < Liquid::Block
    ...
    def render(context)
      ...
      svg_dark  = Jekyll.compile_typst_svg(typst_code, dark,  'typst_input', full_extra, display_math: true, size: dsize)
      svg_light = Jekyll.compile_typst_svg(typst_code, light, 'typst_input', full_extra, display_math: true, size: dsize)
